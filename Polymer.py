...
class Polymer(object):
    ...
    def write_polymer(self,typef="pdb", mypath="."):
        ...
        if all([len(self.poly[j].connect) > 0 for j in range(0,len(self.poly))]):
            self.logger.info(">> Detected connectivity in monomer PDB files, writing out connectivity to PDB<<")
            WriteConnect = True
            C = ''
        else:
            WriteConnect = False
            ...
            for i in range(0,len(data_list),1):
                ...
                if data_list[i][0]!=head_hook and data_list[i][0]!=tail_hook:        
                    ...
                    if  data_list[i][0] == head:
                        ...
                        tail_indx = index
                    if i == 0:
                        indx_shift = index - data_list[i][0]
